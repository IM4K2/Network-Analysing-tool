...
def analyze_packet_data(df):
    ...
    # Fix for percentage calculation to avoid index mismatch
...
def plot_share_of_protocols_between_ips(ip_communication_protocols):
    # Plot the share of protocols between IPs
    plt.figure(figsize=(12, 8))
    for key, grp in ip_communication_protocols.groupby(['Source IP', 'Destination IP']):
        plt.plot(grp['Protocol'], grp['Percentage'], marker='o', label=f"{key[0]} -> {key[1]}")
